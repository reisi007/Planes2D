...
public class BombDrawer implements IDrawable {
	...
	public BombDrawer(UserPlane userPlane, Drawable bomb, float x, float y,
			float currentW) {
		uPlane = userPlane;
		Drawable drawable = new Drawable(bomb);
		drawable.setScale((currentW - bombs.length * 4 - x) / 25f
				/ drawable.getWidth());
		for (int i = 0; i < bombs.length; i++) {
			bombs[i] = new Drawable(drawable);
			bombs[i].setPosition(Anchor.MiddleLeft, i
					* (drawable.getWidth() + 4) + x, y);
		}
	}
	...
	public void Draw(SpriteBatch spriteBatch) {
		drawTo = uPlane.ShotsAvailable() > bombs.length ? bombs.length : uPlane
				.ShotsAvailable();
